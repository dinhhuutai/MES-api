...
const svc = require('./mauphieu.service');
...
const tao = asyncHandler(async (req, res) => {
  ...
  return ok(res, m, 'Đã tạo mẫu phiếu');
});
...
const nhanBan = asyncHandler(async (req, res) => {
  ...
  return ok(res, m, 'Đã nhân bản mẫu phiếu');
});
...
const sua = asyncHandler(async (req, res) => {
  ...
  return ok(res, m, 'Đã lưu mẫu phiếu');
});

const xoa = asyncHandler(async (req, res) => {
  await svc.xoa(req.params.id, req.user.id);
  return ok(res, { id: req.params.id }, 'Đã xóa mẫu phiếu');
});

const gan = asyncHandler(async (req, res) => {
  const r = await svc.gan(req.params.maViTri, (req.body || {}).mau_phieu_id, req.user.id);
  return ok(res, r, r ? 'Đã gắn mẫu vào nút in phiếu' : 'Đã gỡ mẫu — nút in dùng lại bố cục mặc định');
});
