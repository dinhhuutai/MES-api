...
async function getHsktById(id) {
  const { rows } = await query(
    `SELECT id, barcode_hskt, ma_hskt, phuong_an_in, phien_ban, inset, ma_don_ready,
...
// Lịch sử thao tác của HSKT theo barcode (gộp mọi phiên bản).
async function historyByBarcode(barcode) {
  const { rows } = await query(
    `SELECT lh.hanh_dong, lh.chi_tiet, lh.phien_ban, lh.tg, nd.ho_ten AS nguoi, pin.ma_phan
     ...
     WHERE h.barcode_hskt = $1 ORDER BY lh.tg DESC`, [barcode]);
  return rows;
}
...
async function changePhuongAnIn(hsktId, pain, actorId) {
  return withTransaction(async (client) => {
    const { rows: cur } = await client.query(
      'SELECT id, barcode_hskt, phuong_an_in, phien_ban, dang_hoat_dong FROM ho_so_ky_thuat WHERE id=$1', [hsktId]);
    if (!cur.length) return null;
    const h = cur[0];
    if (Number(h.phuong_an_in) === Number(pain)) return h.id; // không đổi
    ...
    await client.query(
      ...
      [newId, `Đổi phương án in ${h.phuong_an_in ?? '—'}→${pain}`, (h.phien_ban || 1) + 1, actorId]);
    return newId;
  });
}
