...
async function list({ search = '', offset = 0, limit = 20 }) {
  const co = await coCotDiaChi();
  const cotDiaChi = co ? 'kh.dia_chi, kh.dia_chi_giao' : 'NULL::text AS dia_chi, NULL::text AS dia_chi_giao';
  // ⚠ Ô tìm soi CẢ địa chỉ (khi có cột) — người dùng hay tra "khách nào ở khu công nghiệp X".
  const dkTim = co
    ? `($1 = '' OR kh.ma_khach_hang ~* $1 OR kh.ten_khach_hang ~* $1 OR COALESCE(kh.dia_chi,'') ~* $1 OR COALESCE(kh.dia_chi_giao,'') ~* $1)`
    : `($1 = '' OR kh.ma_khach_hang ~* $1 OR kh.ten_khach_hang ~* $1)`;
  const FROM = `FROM khach_hang kh WHERE kh.dang_hoat_dong AND ${dkTim}`;
  const args = [mauTim(search)];

  const dataSql = `
    SELECT kh.id, kh.ma_khach_hang, kh.ten_khach_hang, ${cotDiaChi}, kh.ghi_chu,
  ...
  return { rows: data.rows, total: count.rows[0].total, co_cot: co };
}
...
async function getById(id) {
  ...
  const { rows } = await query(
    `SELECT id, ma_khach_hang, ten_khach_hang, ${cot}, ghi_chu FROM khach_hang WHERE id = $1`.replace(/\s+/g, ' '),
    [id]
  );
  return rows[0] || null;
}
...
async function update(id, { diaChi, diaChiGiao, ghiChu }, actorId) {
  const co = await coCotDiaChi();
  const set = [];
  const val = [];
...
module.exports = { list, getById, update, coCotDiaChi };
