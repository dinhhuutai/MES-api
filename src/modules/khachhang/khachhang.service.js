...
// `ten_day_du` là VARCHAR(255) (mig 101) ⇒ trần riêng, khớp đúng độ dài cột.
const DAI_TEN_TOI_DA = 255;
function chuan(v, ten, toiDa = DAI_TOI_DA) {
  if (v === undefined) return undefined;
  const s = String(v == null ? '' : v).trim().replace(/[ \t]+/g, ' ');
  if (s.length > toiDa) {
    throw new AppError(`${ten} quá dài (tối đa ${toiDa} ký tự)`, { status: 422, errorCode: 'QUA_DAI' });
  }
  return s;
}

async function danhSach({ search, page, limit, offset }) {
  const { rows, total, co_cot, co_cot_ten } = await repo.list({ search, offset, limit });
  return { items: rows, meta: { ...buildMeta(page, limit, total), co_cot, co_cot_ten } };
}
...
async function capNhat(id, body = {}, actorId) {
  ...
  if (body.tenDayDu !== undefined && !(await repo.coCotTenDayDu())) {
    throw new AppError('Chưa chạy migration 101 — chưa lưu được tên đầy đủ khách hàng',
      { status: 409, errorCode: 'THIEU_MIGRATION' });
  }

  const data = {
    tenDayDu: chuan(body.tenDayDu, 'Tên đầy đủ', DAI_TEN_TOI_DA),
