...
async function co074(client) {
  if (_co074) return true;
  const { rows } = await client.query(
    `SELECT 1 FROM information_schema.columns WHERE table_name='dot_vai_ve' AND column_name='du_an' LIMIT 1`);
  _co074 = rows.length > 0;
  return _co074;
}
